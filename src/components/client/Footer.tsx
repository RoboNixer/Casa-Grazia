import Link from 'next/link';
import { Phone, Mail, MapPin, MessageCircle } from 'lucide-react';
import type { SiteSettings } from '@/types/database';

export default function Footer({ siteSettings }: { siteSettings: SiteSettings }) {
  const currentYear = new Date().getFullYear();

  // Same brand split as the header (highlight last word in emerald)
  const parts = siteSettings.site_name.trim().split(' ');
  const lead = parts.length > 1 ? parts.slice(0, -1).join(' ') : '';
  const accent = parts[parts.length - 1];

  return (
    <footer className="hero-mediterranean border-t border-emerald-900/[0.07]">
      <div className="relative mx-auto max-w-3xl px-5 sm:px-6 lg:px-8 py-16 lg:py-20 text-center">

        {/* Brand */}
        <div>
          <p className="font-mono text-[10px] tracking-[0.28em] uppercase text-ink-faint mb-2">
            Villa · Istra
          </p>
          <span className="font-display font-medium text-ink text-[26px] sm:text-[30px] leading-none tracking-tight">
            {lead && <span className="italic">{lead} </span>}
            <span className="italic text-emerald-700">{accent}</span>
          </span>
          {siteSettings.site_description && (
            <p className="mt-4 text-[13.5px] leading-relaxed text-ink-muted max-w-xl mx-auto">
              {siteSettings.site_description}
            </p>
          )}
        </div>

        {/* Hairline */}
        <div className="mt-8 mx-auto h-px w-32 bg-gradient-to-r from-transparent via-emerald-900/15 to-transparent" />

        {/* Navigation */}
        <nav className="mt-7 flex flex-wrap items-center justify-center gap-x-7 gap-y-2">
          {[
            { href: '/', label: 'Početna' },
            { href: '/book', label: 'Rezerviraj' },
            { href: '/gallery', label: 'Galerija' },
            { href: '/contact', label: 'Kontakt' },
          ].map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="font-mono text-[10.5px] tracking-[0.2em] uppercase text-ink-muted hover:text-emerald-700 transition-colors duration-200"
            >
              {link.label}
            </Link>
          ))}
        </nav>

        {/* Contact info */}
        <div className="mt-8 flex flex-wrap items-center justify-center gap-x-8 gap-y-3 text-[13.5px] text-ink-muted">
          {siteSettings.address && (
            <div className="inline-flex items-center gap-2">
              <MapPin className="h-4 w-4 shrink-0 text-emerald-700" />
              <span>{siteSettings.address}</span>
            </div>
          )}
          {siteSettings.phone && (
            <a
              href={`tel:${siteSettings.phone}`}
              className="inline-flex items-center gap-2 hover:text-emerald-700 transition-colors"
            >
              <Phone className="h-4 w-4 shrink-0 text-emerald-700" />
              <span>{siteSettings.phone}</span>
            </a>
          )}
          {siteSettings.email && (
            <a
              href={`mailto:${siteSettings.email}`}
              className="inline-flex items-center gap-2 hover:text-emerald-700 transition-colors"
            >
              <Mail className="h-4 w-4 shrink-0 text-emerald-700" />
              <span>{siteSettings.email}</span>
            </a>
          )}
        </div>

        {/* Social */}
        {(siteSettings.facebook_url || siteSettings.instagram_url || siteSettings.whatsapp) && (
          <div className="mt-8 flex items-center justify-center gap-2.5">
            {siteSettings.facebook_url && (
              <a
                href={siteSettings.facebook_url}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center w-10 h-10 rounded-full bg-white/70 backdrop-blur-sm text-ink-muted hover:text-emerald-700 hover:bg-white transition-all duration-200 border border-emerald-900/[0.08] shadow-sm"
                aria-label="Facebook"
              >
                <svg className="h-3.5 w-3.5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                </svg>
              </a>
            )}
            {siteSettings.instagram_url && (
              <a
                href={siteSettings.instagram_url}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center w-10 h-10 rounded-full bg-white/70 backdrop-blur-sm text-ink-muted hover:text-emerald-700 hover:bg-white transition-all duration-200 border border-emerald-900/[0.08] shadow-sm"
                aria-label="Instagram"
              >
                <svg className="h-3.5 w-3.5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/>
                </svg>
              </a>
            )}
            {siteSettings.whatsapp && (
              <a
                href={`https://wa.me/${siteSettings.whatsapp.replace(/\D/g, '')}`}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center w-10 h-10 rounded-full bg-white/70 backdrop-blur-sm text-ink-muted hover:text-emerald-700 hover:bg-white transition-all duration-200 border border-emerald-900/[0.08] shadow-sm"
                aria-label="WhatsApp"
              >
                <MessageCircle className="h-3.5 w-3.5" />
              </a>
            )}
          </div>
        )}

        {/* Bottom bar */}
        <div className="mt-12 pt-7 border-t border-emerald-900/[0.07]">
          <p className="font-mono text-[10.5px] tracking-[0.18em] uppercase text-ink-faint">
            © {currentYear} {siteSettings.site_name} · Sva prava pridržana
          </p>
        </div>
      </div>
    </footer>
  );
}
